<script lang="ts">
  ...
  import { CONFIDENCE_CHIP_CLASSES, CONFIDENCE_LABELS, COPY, formatPercent } from '$lib/copy'
    ...
    <div class="min-w-0">
      {#if detail.current}
        ...
        <p class="flex items-center gap-1.5 text-xs text-gray-500">
          ...
          <span>{COPY.potential.toLowerCase()} {formatPercent(detail.current.composite)}</span>
          <span
          ...
          <li>
            <div class="flex items-baseline justify-between gap-2 text-sm">
              ...
              <span class="shrink-0 font-mono text-xs text-gray-400 tabular-nums">
                {f.subScore === null ? '—' : formatPercent(f.subScore)} · paino {Math.round(f.weight * 100)} %
              </span>
            </div>
